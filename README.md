...
|`LOG_FILES`| A space/comma delimited list of log files that should be captured in the Docker container logs.  Possible log files at the time of this documentation are: `app.log`, `backup_files.log`, `engine_error.log`, `engine_output.log`, `history.log`, `restore_files.log`, and `service.log`.  For more information on log files, see the [CrashPlan documentation](https://support.code42.com/CrashPlan/6/Troubleshooting/Read_Code42_app_log_files). | `history.log` |

...
(Yes, this still works for CrashPlan 7.0.0 - for configuration/monitoring only - [RESTORES FROM THE REMOTE CLIENT WILL NOT WORK](#restores-fail))

# Disclaimers
**IMPORTANT: DO NOT CONTACT CODE42 FOR SUPPORT USING CRASHPLAN WITH THIS DOCKER IMAGE**

*(**TL;DR** = Using CrashPlan in a headless environment may jeopardize your backups - proceed to the [Quick Start](#quick-start) section at your own risk)*
...
|`PUBLIC_PORT`| The TCP port that the Docker container/host machine will be listening on for CrashPlan client/UI connections.  If the default port of 4244 is changed using this varialbe the host port used in the `--publish` parameter must be changed accordingly.  See the [Ports](#ports) section for more information. | `4244` |
